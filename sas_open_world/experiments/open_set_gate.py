"""Entry point for SAS open-set evaluation with a shared OCSVM gate sweep."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Open-set SAS evaluation with per-class One-Class SVM reject gate"
    )
    ...
    parser.add_argument("--ocsvm-nus", default="0.01,0.03,0.05,0.10")
    parser.add_argument("--gate-thresholds", default="-0.50,-0.25,0.00,0.10,0.25")
    parser.add_argument("--coverage-constraint", type=float, default=0.95)
    parser.add_argument("--augment-temporal-diff", action="store_true")
    parser.add_argument("--augment-dynamic-range", action="store_true")
...
def main() -> None:
    ...
    if args.augment_temporal_diff:
        cache_suffix_parts.append("tdiff")
    if args.augment_dynamic_range:
        cache_suffix_parts.append("drange")
    ...
    x_all, label_names = build_feature_cache(
        ...
        augment_temporal_diff=args.augment_temporal_diff,
        augment_dynamic_range=args.augment_dynamic_range,
    ...
    nu_values = [float(part.strip()) for part in args.ocsvm_nus.split(",") if part.strip()]
    ...
    for nu in nu_values:
        for gate_threshold in gate_thresholds:
            confusion_sums[(nu, gate_threshold)] = np.zeros(
    ...
    for repeat in range(args.repeats):
        ...
        for label_name in known_classes:
            ...
            class_train_scaled[label_name] = x_train_scaled[y_train == class_id]
        ...
        for nu in nu_values:
            ocsvm_models = {}
            for label_name in known_classes:
                ocsvm_models[label_name] = OneClassSVM(kernel="rbf", gamma="scale", nu=nu)
            ...
            for gate_threshold in gate_thresholds:
                ...
                confusion_sums[(nu, gate_threshold)] += cm
    ...
    detail_df.to_csv(os.path.join(args.output_dir, "ocsvm_gate_detail.csv"), index=False)
    ...
    summary_df = (
        detail_df.groupby(["nu", "gate_threshold"], as_index=False)
        .agg(**agg_spec)
        .sort_values(
    ...
    summary_df.to_csv(os.path.join(args.output_dir, "ocsvm_gate_summary.csv"), index=False)
    ...
    if feasible_df.empty:
        ...
        key = (float(constrained["nu"]), float(constrained["gate_threshold"]))
        ...
        save_rectangular_confusion(
            ...
            title=f"SAS OCSVM Gate (nu={constrained['nu']:.2f}, thr={constrained['gate_threshold']:.2f})",
        )
    ...
    log(
        f"Finished OCSVM-gate evaluation. "
