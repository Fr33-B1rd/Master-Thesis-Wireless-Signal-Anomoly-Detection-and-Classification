"""Few-shot abnormal subtype classification on top of SAS PatchCore features.

Driver for the supervised fewshot baseline: takes the PatchCore patch artifacts,
aggregates each sample to a single feature vector via
``build_semantic_feature`` (top-K weighted mean and/or projected Gram matrix),
fits a LinearSVC or logistic regression head on K-shot per-class training
splits, and reports averaged accuracy + macro-F1 across repeats.

Moved from the historical ``support/fewshot_patchcore_classifier_sas.py``
when ``support/`` was collapsed; the reusable feature builder
(``build_semantic_feature``) now lives in ``sas_open_world.features.patchcore``
and the cross-package ``support`` import is gone.
"""
...
from cluster_patchcore_sas import build_patch_lookup
from core.utils import log
from dataio.sas import load_sas_test_split
from sas_open_world.features import build_random_projection, build_semantic_feature
...
def main() -> None:
    ...
    test_label_names = load_sas_test_split(dataset_dir, load_images=False).label_names
