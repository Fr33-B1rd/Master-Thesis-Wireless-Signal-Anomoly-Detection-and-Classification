...
from fewshot_patchcore_classifier_sas import build_semantic_feature
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Open-set SAS prototype-similarity reject evaluation"
    )
    ...
    parser.add_argument(
        "--reject-thresholds",
        default="0.50,0.55,0.60,0.65,0.70,0.75,0.80,0.85,0.90,0.95,0.97,0.99",
    )
...
def main() -> None:
    ...
    label_array = np.asarray(test_label_names).astype(str)
    known_index_map = {
    ...
    for repeat in range(args.repeats):
        ...
        X_train_scaled = l2_normalize(X_train_scaled)
        X_eval_scaled = l2_normalize(X_eval_scaled)

        prototypes = []
        for label_name in known_classes:
            class_train_idx = train_idx[label_array[train_idx] == label_name]
            class_vectors = X_train_scaled[np.isin(train_idx, class_train_idx)]
            proto = class_vectors.mean(axis=0, keepdims=True)
            proto = l2_normalize(proto, axis=1)[0]
            prototypes.append(proto)
        prototypes = np.stack(prototypes).astype(np.float32)

        similarity = X_eval_scaled @ prototypes.T
        pred_known_ids = np.argmax(similarity, axis=1)
        pred_conf = np.max(similarity, axis=1)
    ...
    save_rectangular_confusion(
        ...
        title=f"SAS Prototype Reject (shot={args.shot}, thr={best_threshold:.2f})",
    )

    log(
        f"Finished prototype reject eval. "
